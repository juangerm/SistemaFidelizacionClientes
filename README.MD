...
El archivo `src/main/webapp/META-INF/contex.xml` define la conexión a PostgreSQL. Modificar segun corresponda.
...
El archivo `src/main/resources/META-INF/persistence.xml` define la conexión a PostgreSQL. Modificar segun corresponda.
    ...
    ```xml
	<Resource name="jdbc/sifiDS" auth="Container" type="javax.sql.DataSource"
	    username="postgres" password="12345" 
	    driverClassName="org.postgresql.Driver" 
	    url="jdbc:postgresql://localhost:5432/sifidb"/>
...
2. Compilar y ejecutar con Openxava:
	...
	2.5. Ejecutar el script "sifi_db_init.sql" en la base de datos.
	2.6. Una vez hecho eso, abrir el proyecto, se tendrá que ver una ventana al costado izquierdo con la siguiente estructura:
        SistemaFidelizacionClientes/
        ├── src/main/java
        ├── src/main/resources
    2.7. Click derecho encima de src/main/java --> "Run As.." --> Maven Install
	2.8. Click derecho encima de src/main/java --> "Run As.." --> Java Application
	2.9 Se realiza la compilación y ejecución del servicio REST+Frontend.
---
