...
CREATE sequence IF NOT EXISTS public.bolsapunto_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE SEQUENCE IF NOT exists public.conceptopunto_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE SEQUENCE IF NOT exists public.nacionalidades_cod_nacionalidad_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
-- DROP SEQUENCE public.nivelfidelizacion_id_seq;

CREATE SEQUENCE IF NOT EXISTS public.nivelfidelizacion_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
ALTER SEQUENCE public.nivelfidelizacion_id_seq OWNER TO postgres;
GRANT ALL ON SEQUENCE public.nivelfidelizacion_id_seq TO postgres;

-- DROP SEQUENCE public.paises_cod_pais_seq;

CREATE SEQUENCE IF NOT EXISTS public.paises_cod_pais_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
GRANT ALL ON SEQUENCE public.producto_id_seq TO postgres;

-- DROP SEQUENCE public.referircliente_id_seq;

CREATE SEQUENCE IF NOT exists public.referircliente_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
ALTER SEQUENCE public.referircliente_id_seq OWNER TO postgres;
GRANT ALL ON SEQUENCE public.referircliente_id_seq TO postgres;

-- DROP SEQUENCE public.reglaasignacionpunto_id_seq;

CREATE SEQUENCE IF NOT exists public.reglaasignacionpunto_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE SEQUENCE IF NOT exists public.tipos_documentos_cod_tipo_documento_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE SEQUENCE IF NOT EXISTS public.usopuntocabecera_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE SEQUENCE IF NOT EXISTS public.usopuntodetalle_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE SEQUENCE IF NOT EXISTS public.vencimientopunto_id_seq
	INCREMENT BY 1
	MINVALUE 1
	MAXVALUE 2147483647
	START 1
...
CREATE TABLE IF NOT EXISTS public.conceptopunto (
...
-- public.nivelfidelizacion definition

-- Drop table

-- DROP TABLE public.nivelfidelizacion;

CREATE TABLE IF NOT EXISTS public.nivelfidelizacion (
                                                        id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
    beneficios varchar(255) NULL,
    descipcion varchar(255) NULL,
    punto_maximo int4 NULL,
    punto_minimo int4 NULL,
    CONSTRAINT nivelfidelizacion_pkey PRIMARY KEY (id)
    );

-- Permissions

ALTER TABLE public.nivelfidelizacion OWNER TO postgres;
GRANT ALL ON TABLE public.nivelfidelizacion TO postgres;
...
CREATE TABLE IF NOT EXISTS public.producto (
    ...
    precio_referencia numeric(12, 2) NULL,
    activo bool DEFAULT true NULL,
    CONSTRAINT producto_pkey PRIMARY KEY (id),
    CONSTRAINT producto_puntos_requeridos_check CHECK ((puntos_requeridos > 0))
    );
...
CREATE TABLE IF NOT EXISTS public.reglaasignacionpunto (
                                                           id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.vencimientopunto (
                                                       id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
...
END IF;
    v_diff := (NEW.fecha_fin - NEW.fecha_inicio);
    IF NEW.dias_validez IS NOT NULL AND NEW.dias_validez <> v_diff THEN
      RAISE EXCEPTION 'dias_validez debe coincidir con (fecha_fin - fecha_inicio )';
END IF;
END IF;
...
-- public.referircliente definition

-- Drop table

-- DROP TABLE public.referircliente;

CREATE TABLE IF NOT EXISTS public.referircliente (
                                                     id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
    fecharegistro date NULL,
    puntosobtenidos int4 NULL,
    idcliente int4 NULL,
    idreferido int4 NOT NULL,
    idvencimientopunto int4 NULL,
    CONSTRAINT referircliente_pkey PRIMARY KEY (id),
    CONSTRAINT fk_referircliente_cliente FOREIGN KEY (idcliente) REFERENCES public.cliente(id),
    CONSTRAINT fk_referircliente_referido FOREIGN KEY (idreferido) REFERENCES public.cliente(id),
    CONSTRAINT fk_referircliente_vencimientopunto FOREIGN KEY (idvencimientopunto) REFERENCES public.vencimientopunto(id)
    );

-- Permissions

ALTER TABLE public.referircliente OWNER TO postgres;
GRANT ALL ON TABLE public.referircliente TO postgres;
...
CREATE TABLE IF NOT EXISTS public.usopuntocabecera (
                                                       id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
    fecha date NULL,
    ...
    CONSTRAINT fk_usopuntocabecera_producto FOREIGN KEY (idproducto) REFERENCES public.producto(id)
    );
...
AS $function$
DECLARE
v_req   int;   -- puntos requeridos segun concepto
  v_ausar int;   -- puntos a usar
  v_dispo int;   -- saldo disponible del cliente
BEGIN
  -------------------------------------------------------------------
  -- 1) Validación básica
  -------------------------------------------------------------------
  IF NEW.idcliente IS NULL THEN
    RAISE EXCEPTION 'idcliente es obligatorio';
END IF;

  -------------------------------------------------------------------
  -- 2) Caso especial: inserción interna sin concepto ni producto
  --    (por ejemplo, desde fn_usar_puntos con p_id_concepto = NULL
  --     y luego se hace un UPDATE para setear idproducto)
  -------------------------------------------------------------------
  IF NEW.idconceptopunto IS NULL AND NEW.idproducto IS NULL THEN
    -- No forzamos concepto/producto; fn_usar_puntos ya validó saldo y puntos
...
END IF;
  ...
  -------------------------------------------------------------------
  -- 3) Determinar CUÁNTOS PUNTOS se van a usar
  --    MODO CONCEPTO o MODO PRODUCTO
  -------------------------------------------------------------------
  IF NEW.idconceptopunto IS NOT NULL THEN
      --------------------------------------------------------
      -- MODO CONCEPTO
      --------------------------------------------------------
SELECT punto_requerido
INTO v_req
FROM conceptopunto
WHERE id = NEW.idconceptopunto;
...
END IF;
  ...
  ELSIF NEW.idproducto IS NOT NULL THEN
      --------------------------------------------------------
      -- MODO PRODUCTO
      --------------------------------------------------------
      IF NEW.puntaje_utilizado IS NULL THEN
        RAISE EXCEPTION 'Para canje por producto, puntaje_utilizado es obligatorio';
END IF;

      v_ausar := NEW.puntaje_utilizado;

ELSE
      -- Por seguridad, aunque en teoría nunca llega acá
      RAISE EXCEPTION 'Debe indicar idconceptopunto o idproducto';
END IF;

  IF v_ausar <= 0 THEN
    RAISE EXCEPTION 'Los puntos a usar deben ser mayores que 0 (%.).', v_ausar;
END IF;

  -------------------------------------------------------------------
  -- 4) Validar saldo disponible (solo cuando viene desde fuera)
  -------------------------------------------------------------------
...
GRANT ALL ON FUNCTION public.trg_usopuntocabecera_validate() TO public;
...
CREATE TABLE IF NOT EXISTS public.bolsapunto (
                                                 id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
...
CREATE TABLE IF NOT exists public.usopuntodetalle (
                                                      id int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL,
...
-- DROP FUNCTION public.fn_canjear_producto(int4, int4);

CREATE OR REPLACE FUNCTION public.fn_canjear_producto(p_id_cliente integer, p_id_producto integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
v_puntos INT;
    v_id_cab INT;
BEGIN
SELECT puntos_requeridos INTO v_puntos
FROM producto
WHERE id = p_id_producto AND activo = TRUE;

IF v_puntos IS NULL THEN
        RAISE EXCEPTION 'Producto no encontrado o inactivo';
END IF;

    -- usa el consumo FIFO ya implementado
SELECT fn_usar_puntos(p_id_cliente, NULL, v_puntos)
INTO v_id_cab;

-- Asocia la cabecera al producto
UPDATE usopuntocabecera
SET idproducto = p_id_producto
WHERE id = v_id_cab;
...
ALTER FUNCTION public.fn_canjear_producto(int4, int4) OWNER TO postgres;
GRANT ALL ON FUNCTION public.fn_canjear_producto(int4, int4) TO postgres;
...
CREATE OR REPLACE FUNCTION public.fn_usar_puntos(p_id_cliente integer, p_id_concepto integer, p_puntos integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
v_puntos_totales    INT;   -- puntos a descontar en total
    v_restante          INT;   -- lo que falta por descontar
    v_id_cabecera       INT;
    v_fila              RECORD;
BEGIN
    ----------------------------------------------------------------
...
END IF;
    ...
    -- Determinar cuántos puntos usar:
    --  - Si p_puntos viene informado => usamos eso (caso PRODUCTO u otro uso explícito)
    --  - Si p_puntos es NULL => tomamos punto_requerido del conceptopunto (caso clásico)
    IF p_puntos IS NOT NULL THEN
        v_puntos_totales := p_puntos;
ELSE
        IF p_id_concepto IS NULL THEN
            RAISE EXCEPTION 'Debe indicar idconcepto o puntos';
END IF;

SELECT punto_requerido
INTO v_puntos_totales
FROM conceptopunto
WHERE id = p_id_concepto;

IF v_puntos_totales IS NULL THEN
            RAISE EXCEPTION 'Concepto % no encontrado o sin punto_requerido', p_id_concepto;
END IF;
END IF;

    IF v_puntos_totales <= 0 THEN
        RAISE EXCEPTION 'La cantidad de puntos a usar debe ser mayor que cero (%).', v_puntos_totales;
END IF;

    ----------------------------------------------------------------
    -- 2) Verificar saldo suficiente (opcional, pero prolijo)
    ----------------------------------------------------------------
SELECT COALESCE(SUM(saldo_punto), 0)
INTO v_restante
FROM bolsapunto b
         JOIN vencimientopunto v ON v.id = b.idvencimientopunto
WHERE b.idcliente = p_id_cliente
  AND b.saldo_punto > 0
  AND (v.fecha_fin IS NULL OR v.fecha_fin >= CURRENT_DATE);

IF v_restante < v_puntos_totales THEN
        RAISE EXCEPTION 'Saldo insuficiente para cliente %, saldo %, se piden % puntos',
                        p_id_cliente, v_restante, v_puntos_totales;
END IF;

    ----------------------------------------------------------------
    -- 3) Crear cabecera de uso de puntos
    ----------------------------------------------------------------
    v_restante := v_puntos_totales;

INSERT INTO usopuntocabecera(fecha, puntaje_utilizado, idcliente, idconceptopunto)
VALUES (CURRENT_DATE, v_puntos_totales, p_id_cliente, p_id_concepto)
    RETURNING id INTO v_id_cabecera;

----------------------------------------------------------------
-- 4) Recorrer bolsas FIFO y consumir saldo
----------------------------------------------------------------
FOR v_fila IN
SELECT b.id AS id_bolsa,
       b.saldo_punto,
       v.fecha_fin
FROM bolsapunto b
         JOIN vencimientopunto v ON v.id = b.idvencimientopunto
WHERE b.idcliente = p_id_cliente
  AND b.saldo_punto > 0
  AND (v.fecha_fin IS NULL OR v.fecha_fin >= CURRENT_DATE)
ORDER BY v.fecha_fin, b.id
    LOOP
        IF v_restante <= 0 THEN
            EXIT;
END IF;

        IF v_fila.saldo_punto <= 0 THEN
            CONTINUE;
END IF;

        -- cuánto consumo de esta bolsa
        DECLARE
v_consumir INT;
BEGIN
            v_consumir := LEAST(v_fila.saldo_punto, v_restante);

INSERT INTO usopuntodetalle(idbolsapunto, idusopuntocabecera, puntos_consumidos)
VALUES (v_fila.id_bolsa, v_id_cabecera, v_consumir);

UPDATE bolsapunto
SET puntaje_utilizado = COALESCE(puntaje_utilizado, 0) + v_consumir,
    saldo_punto       = saldo_punto - v_consumir
WHERE id = v_fila.id_bolsa;
...
END LOOP;
    ...
    IF v_restante > 0 THEN
        RAISE EXCEPTION 'Error interno: no se pudieron consumir todos los puntos (restan %)', v_restante;
END IF;

RETURN v_id_cabecera;
END;
$function$
;
