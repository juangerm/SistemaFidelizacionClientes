...
public class ServicioRest {
    ...
    /*@GET
    @Path("/EstadisticasResultados")
    @Transactional
    public Response EstadisticasResultados() {

        EntityManager em = XPersistence.getManager();

        List<EstadisticasEncuestaDTO> lista = new ArrayList<>();

        lista.add(obtenerEstadistica(
                em,
        ...
        lista.add(obtenerEstadistica(
                em,
                "¿Qué tan satisfecho está con los beneficios del programa?",
                "respuesta2"));
        ...
        lista.add(obtenerEstadistica(
                em,
                "¿Cómo califica la facilidad de uso?",
                "respuesta4"));

        lista.add(obtenerEstadistica(
                em,
                "¿Tiene sugerencias para mejorar?",
                "respuesta5"));

        return Response.ok(lista).build();
    }

    private EstadisticasEncuestaDTO obtenerEstadistica(
            EntityManager em,
            String pregunta,
            String campoRespuesta) {

        String jpql =
                "SELECT AVG(r.calificacion), COUNT(r) " +
                "FROM Encuesta e " +
                "JOIN e." + campoRespuesta + " r";

        Object[] resultado = (Object[]) em.createQuery(jpql)
                .getSingleResult();

        EstadisticasEncuestaDTO dto = new EstadisticasEncuestaDTO();
        ...
        dto.setPromedio(
                resultado[0] == null
                        ? 0.0
                        : ((Number) resultado[0]).doubleValue());

        dto.setTotalRespuestas(
